...
async function staleCache(lastupdate) {
    let res = await axios.head(`${config.url}/api/v3/obj/announcement?limit=${config.queryLimit}`);
...
function refreshCache() {
    return new Promise((resolve, reject) => {
        axios.get(`${config.url}/api/v3/obj/announcement?limit=${config.queryLimit}`).then(res => {
            try {
