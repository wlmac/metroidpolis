...
function formatDate(date) {
    formattedDate = date.toISOString().replace("T", " ").replace("Z", "-00:00");
    return formattedDate;
}

async function staleCache(lastupdate) {
    let res = await axios.head(`${config.url}/api/v3/obj/announcement?last_modified_date__gte=${formatDate(new Date(new Date() - config.period))}`);
...
function refreshCache() {
    return new Promise((resolve, reject) => {
        axios.get(`${config.url}/api/v3/obj/announcement?last_modified_date__gte=${formatDate(new Date(new Date() - config.period))}`).then(res => {
            try {
    ...
    let result = src.replaceAll(captureRegex, (strmatch, i) => {
        ...
        for (let match of strmatch.matchAll(captureRegex)) {
            image = {
                ...
                url: match.groups.url.startsWith("http") ? match.groups.url : new URL(match.groups.url, `${config.url}`).href
            }
            images.push(image);
        }
    ...
    let interval = setInterval(function () {
        if (!sublist[ctr]) {
            ...
            difflist.forEach(e => {
                ...
                if (e.author && e.organization) {
                    embed.setAuthor({
                        name: `${e.author.first_name} ${e.author.last_name}`,
                        iconURL: `${config.url}${e.organization.icon}`,
                        url: `${config.url}/user/${e.author.username}`
                    });
                } else if (e.author) {
                    embed.setAuthor({
                        name: `${e.author.first_name} ${e.author.last_name}`,
                    ...
                    embed.setAuthor({
                        ...
                        iconURL: `${config.url}${e.organization.icon}`,
                        url: `${config.url}/club/${e.organization.slug}`
                    })
                } else {
                    embed.setAuthor({
                        name: `${e.organization_string}`
                    })
                }
                ...
                embed.setURL(`${config.url}/announcement/${e.id}`);
                embed.setColor(`${e.tags[0] ? e.tags[0].color.slice(1) : '36314a'}`);

                if (e.organization) {
                    embed.addFields({
                        name: `Club`,
                        value: `[${e.organization.name}](${config.url}/club/${e.organization.slug})`,
                        inline: true
                    });
                } else {
                    embed.addFields({
                        name: `Club`,
                        value: e.organization_string,
                        inline: true
                    });
                }
                ...
                if (tags.length > 1024) {
                    ...
                    for (const tag of tags.split(",").map(el => el.trim())) {
                        if (tags_str.length + tag.length > 1021) break;

                        tags_str += `${tag}, `;
                    }

                    tags = tags_str.slice(0, -2) + "...";
                }

                embed.addFields(
                    { name: `Tags`, value: `${tags}`, inline: true },
                    { name: `Create & Edit Date`, value: `Creation: <t:${new Date(`${e.created_date}`).getTime() / 1000 | 0}>\nEdited: <t:${new Date(`${e.last_modified_date}`).getTime() / 1000 | 0}>`, inline: true }
                );

                embed.setFooter({
                    text: 'Metroidpolis Bot by Project Metropolis',
            ...
            if (embedlist.length != 0) {
                ...
                client.channels.fetch(sublist[ctr].channelid).then(channel => {
                    try {
                        for (let i = 0; i < Math.ceil(embedlist.length / 10); i++) {
                            embedListSliced = embedlist.slice(i * 10, (i + 1) * 10)

                            if (pinglist == "") {
                                channel.send({ embeds: embedListSliced });
                            }
                            else {
                                channel.send({ content: pinglist, embeds: embedListSliced });
                            }
                        }
                    }
                    catch { }
